...
describe('PostgreSQL project workflows', () => {
  ...
  it('applies migrations idempotently with tracked checksums', async () => {
    await migrate(pool);
    expect((await pool.query('SELECT name FROM schema_migrations ORDER BY name')).rows.map(row => row.name)).toEqual(['001_foundation.sql', '002_site_intake.sql', '003_property_requirements.sql', '004_project_property_types.sql', '005_ai_provider_routing.sql']);
  });
